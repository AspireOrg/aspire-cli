...
CONFIG_ARGS = [
    ...
    [('--p2sh-dust-return-pubkey',), {'help': 'pubkey to receive dust when multisig encoding is used for P2SH source (default: pubkey from aspire foundation)'}],
...
def main():
    ...
    log.set_up(log.ROOT_LOGGER, verbose=args.verbose, console_logfilter=os.environ.get('ASPIRE_LOGGING', None))
    ...
    if args.action in COMMANDS_WITH_DB or args.action in COMMANDS_WITH_CONFIG:
        init_args = dict(database_file=args.database_file,
                         ...
                         force=args.force, verbose=args.verbose, console_logfilter=os.environ.get('ASPIRE_LOGGING', None),
